...
function renderAudit(events, kinds) {
  ...
  host.innerHTML = `<div class="ahead"><select id="auditKind" aria-label="Filter audit events by kind">${opts}</select> <span class="dim small">sampled audit events (up to 60 per kind; full append-only log on disk at the run's out_dir/events.jsonl)</span></div><div id="auditRows"></div>`;
  const draw = (filter) => {
